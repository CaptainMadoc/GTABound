...
function realInit(dt)
	...
	data["uiShell"] = "/gtabound/ui/ammo.png"
	data["inAccuracy"] = 1
...
function realUpdate(dt)
	...
	if data["fireSelect"] then
		...
		localAnimator.addDrawable({image = "/gtabound/ui/"..data["fireSelect"]..".png", position = vec2.add(vec2.add(activeItemAnimation.ownerAimPosition(), {0, -1}), offset), fullbright = true}, "overlay")	
	end
